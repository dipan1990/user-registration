	...
	public ResponseEntity<?> findByEmail(String email) {
		Optional<User> userData = repository.findByEmail(email);
		 if (userData.isPresent()) {
			 ...
			 return new ResponseEntity(new CustomMessage("user doesn't exist",400,"Bad Request"), HttpStatus.BAD_REQUEST);
	}
		...
		Optional<User> mayExists = repository.findByEmail(user.getEmail());
			if(mayExists.isPresent())
				//throw new BadRequest("User with email "+ user.getEmail() + "already exists");
				 return new ResponseEntity(new CustomMessage("user already exists",400,"Bad Request"), HttpStatus.BAD_REQUEST);
			else 
				user.setActive(1);
	...
	public ResponseEntity<User>  update(String email, User user) {
	    ...
	    if (userData.isPresent()) {
	      ...
	      return new ResponseEntity<>(repository.save(_user), HttpStatus.OK);
	    	...
	    	return new ResponseEntity(new CustomMessage("User does not exist!! Give proper mail id",400,"Bad Request"),HttpStatus.BAD_REQUEST); 
...
//		  }
	...
	public ResponseEntity<?> deactivateByEmail(String email) {
	    ...
	    if (userData.isPresent()) {
	    		...
	    		if(rowsAffected>0) {
	    	    	return new ResponseEntity(new CustomMessage("User successfully deactivated",200,"Success"),HttpStatus.OK); 

	    		}
	    		else
	    	    	return new ResponseEntity(new CustomMessage("User not successfully deactivated",500,"Some server error"),HttpStatus.INTERNAL_SERVER_ERROR); 
	...
	public ResponseEntity<?> login(LoginDto loginDto) {
	    ...
	    if (userData.isPresent()) {
		      User _user = userData.get();
		      if(_user.getPassword().equalsIgnoreCase(loginDto.getPassword())) {
			      return new ResponseEntity(new CustomMessage("Login Successful",200,"Success"), HttpStatus.OK);

		      }
		      else 
		    	  return new ResponseEntity(new CustomMessage("Login Unsuccessful!! password incorrect",400,"Bad Request"),HttpStatus.BAD_REQUEST);
	    	}
	    else 
	    	return new ResponseEntity(new CustomMessage("Login Unsuccessful!! email id incorrect",400,"Bad Request"),HttpStatus.BAD_REQUEST); 
}
}
